...
The repository `roms/` directory contains the complete 168-title MAME 0.289-derived supported set, including two retained homebrew entries, regenerated for native 360x240 and 720x720. Each package is 4,467,392 bytes and total package data is 750,521,856 bytes. To reproduce aggregate SHA-256 `17a70cfd698464cdc020f4a1ae1de4cf0831878be253b6deccca4da16d9b9e67`, sort the bare filenames using ordinal case-sensitive order, emit each file's lowercase SHA-256 followed by two spaces, its filename, and LF, retain the final LF, encode as UTF-8 without a BOM, and SHA-256 that manifest.

These packages store generator provenance `2600ff1`. They were generated while committed `HEAD` was `2600ff1` but the working tree already contained the uncommitted generator and package work, so the field is a nearest-commit label rather than proof that commit `2600ff1` contains this format.
